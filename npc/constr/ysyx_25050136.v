...
(
    ...
    input                                   flush           ,
    input                                   fencei          ,
    (* keep = "true" *) input                                   in_valid_i      ,
    input    [31:0]                         in_pc_i         ,
    input    [31:0]                         in_prepc_i      ,
    input                                   in_taken_i      ,
    input                                   in_btb_hit_i    ,
    (* keep = "true" *) output                                  in_ready_o      ,
    (* keep = "true" *) input                                   out_ready_i     ,
    output   [31:0]                         out_inst_o      ,
    output   [31:0]                         out_pc_o        ,
    output   [31:0]                         out_prepc_o     ,
    output                                  out_taken_o     ,
    output                                  out_btb_hit_o   ,
    (* keep = "true" *) output                                  out_valid_o     ,
    // AXI 接口
    output                                  req_valid_o     ,
    ...
    // 中间信号
    (* keep = "true" *) wire in_fire;
    (* keep = "true" *) wire out_fire;
    wire ready_go;
    wire miss;
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            // 当 flush 有效时，无条件将 ic_idle 置为 1
            if (flush) begin
                ic_idle <= 1'b1; 
            // 当 flush 无效时，再根据 in_fire 和 out_fire 更新
            end else begin
                if (in_fire) begin
                    ic_idle    <= 1'b0;
                    ic_addr    <= in_pc_i;
        ...
        if (reset) begin
            ...
            for (m = 0; m < NUM_WAY; m = m + 1) begin
                for (n = 0; n < NUM_SET; n = n + 1) begin
                    ...
                    cache_tag[m][n]   <= {TAG_WIDTH{1'b0}};
                    cache_data[m][n]  <= {LINE_WIDTH{1'b0}};
                end
            end
        end else begin
            if (flush) begin
