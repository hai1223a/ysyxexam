...
bool isa_difftest_checkregs(CPU_state *ref, uint32_t pc, Vysyx_25050136_NPC *ysyx_25050136_NPC)
{
  for (size_t i = 0; i < REG_NUM; i++)
  {
    if(ref->gpr[i] != ysyx_25050136_NPC->rootp->ysyx_25050136_NPC__DOT__u_ysyx_25050136_RegisterFile__DOT__gpr[i]) return false;
  }
  if(pc != ysyx_25050136_NPC->pc_o) return false;
  return true;
}